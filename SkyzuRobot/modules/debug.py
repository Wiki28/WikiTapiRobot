...
def debug(update: Update, context: CallbackContext):
    ...
    if len(args) > 1:
        if args[1] in ("yes", "on"):
            DEBUG_MODE = True
            message.reply_text("Mode debug sekarang aktif.")
        elif args[1] in ("no", "off"):
            DEBUG_MODE = False
            message.reply_text("Mode debug sekarang nonaktif.")
    else:
        if DEBUG_MODE:
            message.reply_text("Mode debug saat ini aktif.")
        else:
            message.reply_text("Mode debug saat ini tidak aktif.")
